...
int checkUserExist(User u, char*usertype)
{
           ...
           //employee userid and password check
...
void bookedCarDetails1(Customer_Car_Details C)
{
    clrscr();
    ...
    {
        if(strcmp(CC.cust_name,C.cust_name)==0)
        {
            gotoxy(1, x);
...
int rentCar()
{
    ...
    *pos = '\0';
    ...
    if(strcmp(CC.cust_name,"") == 0 || strcmp(CC.drop,"")==0 || strcmp(CC.pick,"") == 0)
            {
                gotoxy(28,18);
                textcolor(LIGHTRED);
                printf("ALL FIELDS ARE MANDATORY");
                _getch();
                gotoxy(28,18);
                printf("\t\t\t\t\t\t\t\t\t");
                return 0;
